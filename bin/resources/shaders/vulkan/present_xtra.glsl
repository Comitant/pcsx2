...
void ps_jittered_stochastic_supersampling()
{
	...
	float rx = nrand(v_tex * u_time);
	float ry = nrand(v_tex * u_time + 0.5);
	
	for(int j = -JSSS_HALF; j <= JSSS_HALF; j++)
	for(int i = -JSSS_HALF; i <= JSSS_HALF; i++) {
		float rx = nrand(v_tex * u_time + i + j);
		float ry = nrand(v_tex * u_time + i + j + 0.5);
...
void ps_quincunx()
{
	...
	tmpcol = sample_c(v_tex + vec2( 0.25, 0.25) * dxy).rgb; color += GAMMA_PRE(tmpcol);
	tmpcol = sample_c(v_tex + vec2( 0.25,-0.25) * dxy).rgb; color += GAMMA_PRE(tmpcol);
	tmpcol = sample_c(v_tex + vec2(-0.25,-0.25) * dxy).rgb; color += GAMMA_PRE(tmpcol);
	tmpcol = sample_c(v_tex + vec2(-0.25, 0.25) * dxy).rgb; color += GAMMA_PRE(tmpcol);
	tmpcol = sample_c(v_tex).rgb;
	
	color = color * 0.125 + GAMMA_PRE(tmpcol) * 0.5;

	o_col0 = vec4(GAMMA_POST(color), 1);
}
#endif
...
void ps_4x_rgss()
{
	...
	tmpcol = sample_c(v_tex + vec2( s, l) * dxy).rgb; color += GAMMA_PRE(tmpcol);
	tmpcol = sample_c(v_tex + vec2( l,-s) * dxy).rgb; color += GAMMA_PRE(tmpcol);
	tmpcol = sample_c(v_tex + vec2(-s,-l) * dxy).rgb; color += GAMMA_PRE(tmpcol);
	tmpcol = sample_c(v_tex + vec2(-l, s) * dxy).rgb; color += GAMMA_PRE(tmpcol);
		   
	o_col0 = vec4(GAMMA_POST(color * 0.25),1);
}
#endif
...
void ps_nearest_aa()
{
	vec2 uv = v_tex * u_source_resolution + 0.5;
	vec2 fl = floor(uv);
	vec2 fr = fract(uv);
	vec2 aa = fwidth(uv) * MAGIC;
	fr = smoothstep(0.5 - aa, 0.5 + aa, fr);
	
	o_col0 = sample_c((fl+fr-0.5)*u_rcp_source_resolution);
}
#endif
